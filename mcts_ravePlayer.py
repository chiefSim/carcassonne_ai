...
class MCTS_RAVEPlayer(Player):
    ...
    def __init__(self, iterations = 1000, timeLimit = 10, isTimeLimited = True, c_param = 4):
        super().__init__()
    ...
    def MCTS_TimeLimit(self, root_node, root_state):
        ...
        if 'MCTSStats.csv' in os.listdir('logs'):
            ...
            dfNew = {'Name': self.name,'TimeLimit':self.timeLimit,'Turn':int((root_state.Turn+1)/2), 'Iter':numberOfIterations}
            dfMCTS = dfMCTS.append(dfNew, ignore_index = True)
            dfMCTS.to_csv(r'logs\MCTSStats.csv', index=False)  # export
