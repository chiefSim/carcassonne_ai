...
class Star2_5(Player):
    ...
    def __init__(self, currentPlayer = 0, Depth = 0, MaxDepth = 1, Alpha = -float('inf'), Beta = float('inf'), ProbingFactor = 5, Chance = None):
        super().__init__()
    ...
    def chooseAction(self, state):
        ...
        if 'ExpectimaxStats.csv' in os.listdir('logs'):
            dfMCTS = pd.read_csv('logs/ExpectimaxStats.csv')
            dfNew = {'Name': self.name,'MaxDepth':self.MaxDepth,'ProbingFactor': self.ProbingFactor,
                     'Turn':int((state.Turn+1)/2), 'Time': time.time()-startTime, 'NodesVisited':VN}
            dfMCTS = dfMCTS.append(dfNew, ignore_index = True)
            dfMCTS.to_csv(r'logs\ExpectimaxStats.csv', index=False)  # export
