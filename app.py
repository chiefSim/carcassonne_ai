...
if 'FinalLeagueTable.csv' in os.listdir('logs'):
    ...
    dfLeague = pd.read_csv('logs/test_FinalLeagueTable.csv')
...
if 'PlayerStats.csv' in os.listdir('logs'):
    ...
    dfStats = pd.read_csv('logs/test_PlayerStats.csv', converters={'MeepleFeatures': eval, 'MeepleTurns': eval})    
    
    
if 'ExpectimaxStats.csv' in os.listdir('logs'):
    dfMax = pd.read_csv('logs/ExpectimaxStats.csv')
else:
    dfMax = pd.read_csv('logs/test_ExpectimaxStats.csv')

if 'MCTSStats.csv' in os.listdir('logs'):
    dfMCTS = pd.read_csv('logs/MCTSStats.csv')
else:
    dfMCTS = pd.read_csv('logs/test_MCTSStats.csv')    
...
dfAvgMeepleScores = dfAvgMeepleScores.round(2)
# feature types
conditions = [
...
"""
    2.1. Feature Scores - Bottom-left (Percent)
"""
# percentages
...
dfMeepleScoresPercent = dfMeepleScoresPercent.round(2)
# feature types
conditions = [
...
timeLimit = dfMCTS['TimeLimit'].values[0]
dfMCTS_fig = dfMCTS[['Name','Turn','Iter']].groupby(['Name','Turn'], as_index=False).mean()

fig_MCTS = px.line(dfMCTS_fig, x="Turn", y="Iter", color="Name",
                   line_group="Name", hover_name="Iter",
                   labels = {
                     "Turn": "Turn",
                     "Iter": "Avg. Number of Iterations"
                     },
                   title = f'Avg. Number of Iterations per Turn (Time Limit = {timeLimit} secs)')
...
"""
    3. Meeple Placement - middle-right
"""    
# Meeple Placement
...
dfAvgPlacementsFinal = dfAvgPlacementsFinal.round(2)
# fix feature names
conditions = [
...
"""
    3.1. Meeple Placement - bottom-right (percent)
"""
...
dfAvgPlacementsPercent = dfAvgPlacementsPercent.round(2)
# fix feature names
conditions = [
                               ...
                               children=[
                                  ...
                                  html.Div(className='five columns div-for-charts bg-grey',   # Define the right element
                                           children = [
                                               ...
                                               dcc.Dropdown(id="Score", options=[{'label':'Total','value':'Total'},{'label':'Percent','value':'Percent'}],value='Total',clearable=False),
...
def update_bar_chart(Players, Score):
    ...
    if Score == 'Total':
        df = dfAvgMeepleScores
        title = "Average Scores Per Feature"
        label = "Avg. Scores"
    elif Score == 'Percent':
        df = dfMeepleScoresPercent[dfMeepleScoresPercent['Feature'] != 'Total']
